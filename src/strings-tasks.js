...
function sumOfCodes(str) {
  ...
  const summOfCodes = [...str].reduce((sum, char) => {
...
function countVowels(str) {
  const countVowel = str.match(/[aeiouyAEIOUY]/g);
  return countVowel ? countVowel.length : 0;
}
...
function findLongestWord(sentence) {
  ...
  return words.reduce((longWord, word) => {
    ...
    return cleanWord.length > longWord.length ? cleanWord : longWord;
  }, '');
}
...
function extractNameFromTemplate(value) {
  const NameFromTemplate = value.match(/Hello, (.+?)!/);
  return NameFromTemplate ? NameFromTemplate[1] : '';
}
...
function encodeToRot13(str) {
  ...
  function getAlphabetStart(char) {
    return isUpperCase(char) ? 'A'.charCodeAt(0) : 'a'.charCodeAt(0);
  }
  function rot13Char(char) {
    const alphabetStart = getAlphabetStart(char);
    const charCode = char.charCodeAt(0);
    if (/[A-Za-z]/.test(char)) {
      return String.fromCharCode(
        ((charCode - alphabetStart + 13) % 26) + alphabetStart
      );
    }
    return char;
  }

  // Use the defined function to encode each character in the string
